...
public sealed partial class IncrementalPullConsumer(
    IJobRepository jobRepository,
    IVSphereClient vSphereClient,
    StorageAdapterFactory storageFactory,
    INotificationService notifications,
    ILogger<IncrementalPullConsumer> logger) : IConsumer<IncrementalPullMessage>
{
    public async Task Consume(ConsumeContext<IncrementalPullMessage> context)
    {
        ...
        using var telemetry = WorkerTracing.StartStepActivity(
            nameof(IncrementalPullConsumer), step, msg.JobId, msg.StepId, msg.CorrelationId);
        ...
        try
        {
            // TODO: Query CBT changed blocks since msg.ChangeId
            _ = vSphereClient; // Will call QueryChangedDiskAreasAsync in Phase C
            _ = storageFactory; // Will upload delta artifact in Phase C
    ...
    private async Task FailStepAsync(
        ...
        string error,
        IncrementalPullMessage msg,
        ConsumeContext<IncrementalPullMessage> context,
    ...
    [LoggerMessage(Level = LogLevel.Information, Message = "IncrementalPull starting for Job {JobId}, Step {StepId}")]
    ...
    [LoggerMessage(Level = LogLevel.Error, Message = "IncrementalPull failed for Job {JobId}, Step {StepId}")]
