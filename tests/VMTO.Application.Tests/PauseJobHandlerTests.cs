...
/// <summary>
/// PauseJobHandler 的單元測試。
/// 驗證暫停工作的各種情境。
/// </summary>
public sealed class PauseJobHandlerTests
{
    private readonly IJobRepository _jobRepository = Substitute.For<IJobRepository>();
    private readonly PauseJobHandler _handler;
    ...
    public PauseJobHandlerTests()
    {
        _handler = new PauseJobHandler(_jobRepository);
    }
    ...
    public async Task HandleAsync_找不到Job回傳錯誤()
    {
        ...
        var result = await _handler.HandleAsync(new PauseJobCommand(jobId));
    ...
    [Fact]
    public async Task HandleAsync_無法暫停非Running狀態的Job()
    {
